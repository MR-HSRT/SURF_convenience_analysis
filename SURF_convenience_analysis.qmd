...
acc_pre_exp <- daten_analysis |>
  ...
  dplyr::mutate(
    ...
    # knapp über dem oberen CI-Ende, aber mit Deckel
    y_label = pmin(ymax + 0.10, 4.28)
  ) |>
...
p_acc_pre_exp <- ggplot2::ggplot(
  ...
  ggplot2::geom_text(
    ggplot2::aes(y = y_label, label = label),
    colour = SURF_TEXT,
    size = 3.1,
    vjust = 0
  ) +
  ...
  # WICHTIG: ylim via coord_cartesian + clip off (gegen Abschneiden)
  ggplot2::coord_cartesian(ylim = c(0, 4.35), clip = "off") +
  ggplot2::labs(
  ...
  ggplot2::theme(
    # mehr Platz oberhalb, damit Text außerhalb des Panels auch sichtbar bleibt
    plot.margin = ggplot2::margin(16, 12, 18, 12)
  )
