...
```{r}
#| label: plot-02-01-acc-pre-groups1
#| warning: false
#| message: false
...
acc_pre_exp <- daten_analysis |>
  ...
  dplyr::summarise(mean_ci_n(acc_pre), .groups = "drop") |>
  dplyr::arrange(device_group, rand_freq)
...
p_acc_pre_exp <- ggplot2::ggplot(
  ...
  add_point_labels_left(x_nudge = -0.15, digits = 2, size = 2.9) +
  ...
  ggplot2::scale_x_discrete(
    drop = FALSE,
    expand = ggplot2::expansion(mult = c(0.16, 0.06))
  ) +
  ggplot2::labs(
...
save_plot(
  plot  = p_acc_pre_exp_branded,
  block = 2,
  sub   = 2,
  slug  = "acceptance_pre_experiment_groups"
)
...
p_acc_post_groups <- ggplot2::ggplot(
  acc_post_groups,
...
save_plot(p_acc_post_groups_branded, 2, 2, "acceptance_post_groups")
...
acc_post_exp <- daten_analysis |>
  ...
  dplyr::filter(!is.na(device_group), !is.na(rand_freq)) |>
  dplyr::group_by(device_group, rand_freq) |>
  dplyr::summarise(mean_ci_n(acc_post), .groups = "drop") |>
  dplyr::arrange(device_group, rand_freq)
...
p_acc_post_exp <- ggplot2::ggplot(
  acc_post_exp,
  ggplot2::aes(x = rand_freq, y = mean, group = 1)
) +
  ggplot2::geom_point(colour = SURF_PURPLE, size = 2.8) +
  ggplot2::geom_errorbar(
    ggplot2::aes(ymin = ymin, ymax = ymax),
...
save_plot(p_acc_post_exp_branded, 2, 3, "acceptance_post_experiment_groups")
...
daten_analysis <- daten_analysis |>
  dplyr::mutate(
    # Besitzstatus: nur echte Gerätebesitzer gelten als "Besitzer"
    ...
    # explizite Faktor-Reihenfolge (wichtig für pivot_wider & Plots)
    owner_group = factor(
      owner_group,
      levels = c("Gerätebesitzer", "Nicht-Besitzer")
    ),
    
    # Differenz der Partizipationsintention (Post – Pre)
    part_diff = dplyr::if_else(
      !is.na(part_post_real) & !is.na(part_pre_real),
      part_post_real - part_pre_real,
      NA_real_
    )
  )
...
part_pre_groups <- daten_analysis |>
  ...
  dplyr::summarise(mean_ci_n(part_pre_real), .groups = "drop")
...
part_post_groups <- daten_analysis |>
  ...
  dplyr::group_by(owner_group) |>
  dplyr::summarise(mean_ci_n(part_post_real), .groups = "drop")
