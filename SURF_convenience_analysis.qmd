...
In einem zweiten Schritt werden Fälle entfernt, die Auffälligkeiten im Hinblick auf diese Datenqualitätsmerkmale aufweisen. In Summe wurden auf diese Weise somit XXX Fälle von den folgenden Analysen ausgeschlossen.

### Verteilungs-Analyse: Bearbeitungszeit

In einem ersten Schritt werden solche Fälle identifiziert, die eine extreme Bearbeitungsdauer der Umfrage aufweisen. Diese Ausreißer der Bearbeitungsdauer können beispielsweise von geöffneten und nicht abgeschlossenen Browser-Fenstern herrühren, wobei die Umfrage zu einem späteren Zeitpunkt abgeschlossen wurde. Insgesamt wurden xxx solcher Fälle identifiziert.
...
#### Bearbeitungszeit: Histogramm mit Ausreissern

Zunächst wird die Bearbeitungsdauer aller Fälle, wie unten dargestellt, inspiziert. Ausreißer können so zunächst visuell und dann formal definiert identifiziert werden, um sie von anschließenden Analysen des Speedings auszuschließen. Verzerrungen durch derartige Ausreißer würden ansonsten das relative Kriterium 
...
#### Bearbeitungszeit ohne Ausreisser

```{r}
#| label: qc-speeding-clean

median_time_clean <- median(
  surf_check$interview_min[!surf_check$flag_extreme_slow],
  na.rm = TRUE
)

cutoff_speeder_clean <- median_time_clean / 3

surf_check <- surf_check |>
  dplyr::mutate(
    flag_speeding_clean = interview_min < cutoff_speeder_clean & !flag_extreme_slow
  )

table(surf_check$flag_speeding_clean)
median_time_clean
cutoff_speeder_clean

```

```{r}
#| label: qc-time-plot-without-outliers

surf_plot <- surf_check |> dplyr::filter(!flag_extreme_slow)

p_without <- ggplot(surf_plot, aes(x = interview_min)) +
  geom_histogram(bins = 30) +
  geom_vline(xintercept = median_time_clean, linetype = "dashed") +
  geom_vline(xintercept = cutoff_speeder_clean, linetype = "dashed") +
  labs(
    title = "Verteilung der Bearbeitungszeit (ohne extrem langsame Ausreißer)",
    x = "Bearbeitungszeit (Minuten)",
    y = "Anzahl Befragte"
  ) +
  annotate(
    ...
    y = -3,
    label = paste0(
      "**Eingezeichnet:** Median = ", round(median_time_clean, 1),
      " min | Speeder-Cutoff = ", round(cutoff_speeder_clean, 1), " min"
    ),
    vjust = 1.5,
    hjust = 0.5,
    size = 3.8,
...
ggsave(file.path(PLOTS_DIR, "bearbeitungszeit_ohne_ausreisser.png"),
       p_without, width = 8, height = 5, dpi = 300)
...
#### Bearbeitungszeit mit Ausreissern

```{r}
#| label: qc-time-distribution

# Bearbeitungszeit in Minuten
surf_check <- surf_check |>
  dplyr::mutate(interview_min = interviewtime / 60)

# Kennzahlen
median_min <- median(surf_check$interview_min, na.rm = TRUE)
q <- quantile(surf_check$interview_min, probs = c(.05, .25, .50, .75, .95), na.rm = TRUE)
cutoff_speeder <- median_min / 3

# Plot: Verteilung + Quantile + Median + Speeder-Cutoff + Text-Annotation
p_time <- ggplot2::ggplot(surf_check, ggplot2::aes(x = interview_min)) +
  ggplot2::geom_histogram(bins = 30) +
  ggplot2::geom_vline(xintercept = as.numeric(q), linetype = "dotted") +
  ggplot2::geom_vline(xintercept = median_min, linetype = "dashed") +
  ggplot2::geom_vline(xintercept = cutoff_speeder, linetype = "dashed") +
  ggplot2::labs(
    title = "Verteilung der Gesamtbearbeitungszeit",
    subtitle = "Eingezeichnet: Quantile (gepunktet), Median (gestrichelt), Speeder-Kriterium (< 1/3 Median; gestrichelt)",
    x = "Bearbeitungszeit (Minuten)",
    y = "Anzahl Befragte"
  ) +
  ggplot2::theme_minimal(base_size = 12) +
  ggplot2::annotate(
    ...
    label = paste0(
      "Median: ", round(median_min, 2), " min\n",
      "Cutoff (Speeder): < ", round(cutoff_speeder, 2), " min\n",
      "Q05: ", round(q[1], 2), " | Q25: ", round(q[2], 2), "\n",
      "Q50: ", round(q[3], 2), " | Q75: ", round(q[4], 2), "\n",
      "Q95: ", round(q[5], 2)
    )
  )

# Export für PowerPoint/Report
ggplot2::ggsave(
  filename = file.path(PLOTS_DIR, "qc_bearbeitungszeit_verteilung_mit_cutoffs.png"),
  plot = p_time,
  width = 8, height = 5, dpi = 300
)
...
Straightlining bezeichnet Antwortmuster mit weitgehend konstanten Angaben innerhalb von Likert-Batterien (z.B. durchgängig dieselbe Ausprägung) und kann auf geringe Sorgfalt beim Ausfüllen hindeuten. In dieser Analyse wird Straightlining als Standardabweichung (SD) = 0 über die ausgewählten Likert-Items operationalisiert, wobei zusätzlich vorausgesetzt wird, dass mindestens ein definierter Anteil der Items tatsächlich beantwortet wurde. Im Folgenden werden Häufigkeit und Verteilung der so identifizierten Fälle berichtet.


XXXX --> hier weiter mit GPT
...
surf_check <- surf_check |>
dplyr::mutate(
n_answered_scales = rowSums(!is.na(dplyr::across(dplyr::all_of(scale_vars)))),
straightline_sd   = apply(dplyr::select(., dplyr::all_of(scale_vars)), 1, sd, na.rm = TRUE),
...
#| label: qc-straightlining-plot

p_sl <- surf_check |>
  dplyr::mutate(Straightlining = ifelse(flag_straightline, "Straightliner", "Keine Straightliner")) |>
  ggplot(aes(x = Straightlining)) +
  geom_bar() +
  labs(
    title = "Straightlining in der Stichprobe",
    y = "Anzahl Befragte",
    x = ""
  ) +
  theme_minimal()

ggsave(filename = file.path(PLOTS_DIR, "straightlining_distribution.png"),
       plot = p_sl, width = 6, height = 4)

p_sl

```

Gemäß der hier angewendenten Qualitätsstandards für die erhobenen Daten, können somit alle Befragten im Datensatz in die folgenden Analysen einbezogen werden.
