...
```{r}
#| label: block01-devices-overview
#| warning: false
#| message: false
...
# Save-Plot Engine (einfach, robust)
save_plot <- function(plot, block, sub, slug,
                      width = 8, height = 5, dpi = 300) {
  filename <- sprintf("%02d_%02d_%s.png", block, sub, slug)
  path <- file.path(PLOTS_DIR, filename)
...
# 6) Plot (100%-Stacked), SURF-Style, Logo, Export
plot_df <- device_table |>
  dplyr::mutate(Anteil_prop = Anteil / 100)
...
p_b01 <- ggplot2::ggplot(
  ...
  ggplot2::theme(
    legend.position = "bottom",
    legend.box = "vertical",
    legend.text = ggplot2::element_text(colour = SURF_TEXT)
  )
...
```{r}
#| label: plot-01-01-devices-overview
#| warning: false
#| message: false
...
device_groups <- device_group_flags |>
  dplyr::mutate(
    device_group = dplyr::case_when(
      has_device ~ "Aktive Flexibilitätsnutzer",
...
acc_pre_groups <- daten_analysis |>
  dplyr::bind_rows(daten_analysis |> dplyr::mutate(device_group = "Gesamtstichprobe")) |>
  dplyr::group_by(device_group) |>
  dplyr::summarise(mean_ci(acc_pre), .groups = "drop")
...
p_acc_pre_groups <- ggplot2::ggplot(acc_pre_groups, ggplot2::aes(x = device_group, y = mean)) +
  ...
  ggplot2::geom_errorbar(ggplot2::aes(ymin = ymin, ymax = ymax),
                         width = 0.15, colour = SURF_PURPLE) +
  add_point_labels(digits = 2, size = 3.1, vjust = -1) +
  ggplot2::labs(
...
p_acc_pre_exp <- ggplot2::ggplot(acc_pre_exp, ggplot2::aes(x = rand_freq, y = mean)) +
  ggplot2::geom_point(colour = SURF_PURPLE, size = 2.8) +
  ggplot2::geom_errorbar(ggplot2::aes(ymin = ymin, ymax = ymax),
                         width = 0.2, colour = SURF_PURPLE) +
  add_point_labels(digits = 2, size = 2.9, vjust = -1) +
  ...
  ggplot2::labs(
    ...
    x = "Experimentalbedingung (Häufigkeit der Steuerung)", y = "Akzeptanz (Mittelwert)"
  ) +
...
### 022 Akzeptanz am Ende

```{r}
#| label: plot-02-02-acc-post-groups
#| warning: false
#| message: false
...
acc_post_groups <- daten_analysis |>
  dplyr::bind_rows(daten_analysis |> dplyr::mutate(device_group = "Gesamtstichprobe")) |>
  dplyr::group_by(device_group) |>
  dplyr::summarise(mean_ci(acc_post), .groups = "drop")
...
save_plot(p_acc_post_groups_branded, 2, 1, "acceptance_post_groups")
p_acc_post_groups_branded

```

```{r}
#| label: plot-02-02-acc-post-experiment
#| warning: false
#| message: false
...
p_acc_post_exp <- ggplot2::ggplot(acc_post_exp, ggplot2::aes(x = rand_freq, y = mean)) +
  ggplot2::geom_point(colour = SURF_PURPLE, size = 2.8) +
  ggplot2::geom_errorbar(ggplot2::aes(ymin = ymin, ymax = ymax),
                         width = 0.2, colour = SURF_PURPLE) +
  add_point_labels(digits = 2, size = 2.9, vjust = -1) +
...
save_plot(p_acc_post_exp_branded, 2, 2, "acceptance_post_experiment_groups")
p_acc_post_exp_branded

```

### 023 Akzeptanz Differenz Beginn-Ende
...
acc_diff_groups <- daten_analysis |>
  dplyr::bind_rows(daten_analysis |> dplyr::mutate(device_group = "Gesamtstichprobe")) |>
  dplyr::group_by(device_group) |>
  dplyr::summarise(mean_ci(acc_diff), .groups = "drop")
...
save_plot(p_acc_diff_groups_branded, 2, 1, "acceptance_diff_groups")
p_acc_diff_groups_branded

```

```{r}
#| label: plot-02-03-acc-diff-experiment
#| warning: false
#| message: false
...
save_plot(p_acc_diff_exp_branded, 2, 2, "acceptance_diff_experiment_groups")
p_acc_diff_exp_branded

```

## 03 Partizipationsintention (PI)
...
daten_analysis <- daten_analysis |>
  dplyr::mutate(
    owner_group = dplyr::if_else(
      device_group == "Aktive Flexibilitätsnutzer",
      "Gerätebesitzer",
      "Nicht-Besitzer"
    ),
    owner_group = factor(
      owner_group,
      levels = c("Gerätebesitzer", "Nicht-Besitzer")
    ),
    part_diff = part_post_real - part_pre_real
  )
...
part_pre_groups <- daten_analysis |>
  ...
  dplyr::summarise(mean_ci(part_pre_real), .groups = "drop")
